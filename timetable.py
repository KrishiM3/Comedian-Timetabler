...
class Timetable:
	...
	#Small utility method to check if a comedian can be allocated to a demographic
		...
		if not isTest:
			...
			for t in topics:
				if t not in comedian.themes:
					print(str(comedian.name) + " cannot be allocated to demographic " + str(demographic.reference))
					return False
	...
	def task1Checker(self, comedian_List, demographic_List):
		...
		for day in self.schedule:
			...
			for entry in self.schedule[day]:
				...
				#Make sure that every demographic is only allocated to once a week
				if demographic.reference in demographic_Assigned:
					print(str(demographic.reference) + " is being allocated to more than once a week.")
					return False
				else:
					demographic_Assigned.append(demographic.reference)

				#We make sure every comedian is only allocated to a single show a day
				...
				#Finally, we make sure that the comedian in each show can be allocated to the assigned demographic
				if not self.canMarket(comedian, demographic, False):
					print(str(comedian.name) + " can not be allocated to demographic " + str(demographic.reference) + ", their themes do not match the topics.")
					return False
			...
			for entry in self.schedule[day]:
				...
				if show_type == "main":
					if demographic.reference in main_demographics_Assigned:
						print(str(demographic.reference) + " is being allocated more than one main show a week.")
						return False
					else:
					...
					if demographic.reference in test_demographics_Assigned:
						print(str(demographic.reference) + " is being allocated more than one test show a week.")
						return False
					else:
						test_demographics_Assigned.append(demographic.reference)

				#We make sure that an illegal session type has not been entered somehow
				else:
				...
				#check if the comedian can be allocated to the assigned demographic
